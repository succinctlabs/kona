...
#[derive(Debug, Clone)]
pub struct OracleL2ChainProvider<O: PreimageOracleClient> {
...
impl<O> OracleL2ChainProvider<O>
where
...
impl OracleL2ChainProvider<InMemoryOracle> {
...
impl OracleL2ChainProvider<CachingOracle> {
    /// Returns a [Header] corresponding to the given L2 block number, by walking back from the
    /// L2 safe head.
    async fn header_by_number(&mut self, block_number: u64) -> Result<Header> {
        // Fetch the starting L2 output preimage.
...
#[async_trait]
impl L2ChainProvider for OracleL2ChainProvider<InMemoryOracle> {
...
#[async_trait]
impl L2ChainProvider for OracleL2ChainProvider<CachingOracle> {
    async fn l2_block_info_by_number(&mut self, number: u64) -> Result<L2BlockInfo> {
        // Get the payload at the given block number.
        let payload = self.payload_by_number(number).await?;
...
impl TrieDBFetcher for OracleL2ChainProvider<InMemoryOracle> {
...
impl TrieDBFetcher for OracleL2ChainProvider<CachingOracle> {
    fn trie_node_preimage(&self, key: B256) -> Result<Bytes> {
